...
struct HardwareKeyboardInput: UIViewRepresentable {
        ...
        func insertText(_ text: String) {
            if text == "\n" {
                owner.game.submit(
                    ...
                    hardMode: owner.isHardMode,
                    toastMessageCenter: owner.toastMessageCenter
                )
                return
            }

            let locale = owner.game.expected.locale
            ...
            owner.game.insertText(
                letter: MultiCharacterModel(character)
            )
        }

        func deleteBackward() {
            owner.game.deleteBackward()
        }
    }
    ...
    func updateUIView(_ uiView: InternalView, context: Context) {
        if uiView.handledFocusRequests != focusRequests {
            uiView.handledFocusRequests = focusRequests
