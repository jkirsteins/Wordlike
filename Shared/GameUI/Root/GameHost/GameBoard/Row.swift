...
struct Row: View {
    ...
    @EnvironmentObject var boardReveal: BoardRevealModel
    ...
    func maskedModel(at ix: Int) -> TileModel? {
        guard let model = model.char(guessAt: ix)
        else {
            return nil
        }
    ...
    var isUnknownWord: Bool {
        ...
        return validator.guessTree?.contains(
    ...
    var body: some View {
        ...
        .onReceive(
            vm.$localRevealedCount.debounce(for: 0.1, scheduler: DispatchQueue.main)
        ) {
            nrc in
            guard nrc == 5
            else {
            ...
            guard self.model.expected == self.model.word
            else {
...
private struct SubmittableRow_Preview: View {
...
private struct InvalidSubmittableRow_Preview: View {
    @State var model = RowModel(
        word: WordModel("fuels", locale: .en_US),
        expected: WordModel("fuels", locale: .en_US),
        isSubmitted: false
