...
struct KeyboardHints {
    ...
    let hints: [CharacterModel: TileBackgroundType]

    /// Locale is used to generate the alphabet, so
    /// we can infer which are remaining usable chars.
    let locale: GameLocale

    init(hints: [CharacterModel: TileBackgroundType], locale: GameLocale) {
        self.hints = hints
        self.locale = locale
    }

    init() {
        self.hints = [CharacterModel: TileBackgroundType]()
        self.locale = .en_US
    }

    init(hints: [String: TileBackgroundType], locale: GameLocale) {
...
struct EditableRow: View {
    ...
    @State var background: Color = .init(NativeColor.systemFill)

    var body: some View {
        let showFocusHint = hasHardwareKeyboard && editable && (isActive == tag)
