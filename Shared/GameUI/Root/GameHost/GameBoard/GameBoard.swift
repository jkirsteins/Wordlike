...
struct GameBoard: View {
    ...
    let earlyCompleted: ((GameState) -> Void)?
    let completed: ((GameState) -> Void)?
...
private struct InternalPreview: View {
    @State var state = GameState(expected: TurnAnswer(
        word: "board",
        day: 1,
        locale: .en_US,
    ...
    var body: some View {
        VStack {
            ...
            Button("Reset") {
                self.state = GameState(expected: TurnAnswer(
                    word: "fuels",
                    day: 1,
                    locale: .en_US,
                    validator: WordValidator(locale: .en_US)
