...
extension GameState {
    func calculateKeyboardHints(from newRows: [RowModel]) -> KeyboardHints {
        var result: [CharacterModel: TileBackgroundType] = [:]
        ...
        for srow in submittedRows {
            for ix in 0 ..< srow.word.count {
                ...
                guard let chars = srow.char(guessAt: ix)
                else {
                    continue
                }
    ...
    var body: some View {
        GeometryReader { gr in
            VStack {
                ...
                HStack {
                    Text(
                        "Hints should higlight simplified as yellow when answer contains non-simplified in a different position."
                    )
                    Spacer()
                }
    ...
    var body: some View {
        GeometryReader { gr in
            VStack {
                ...
                HStack {
                    Text(
                        "If we switch between hard/easy during a round, we can end up in a situation where a letter pairing is uncertain (e.g. Ē has been tried and is invalid, but E hasn't and is valid).\n\nWe should not mark these letters as 'known' in this case.\n\nE should be 'unknown' and neither green nor dark."
                    )
                    Spacer()
                }
                Divider()
