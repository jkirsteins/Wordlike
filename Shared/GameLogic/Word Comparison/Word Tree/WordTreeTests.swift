        ...
        TestList("WordTree tests") {
            Group {
                Test(
                    ...
                    Text(verbatim: "Retrieved fuel (as a subset of fuels): \(data.4?.displayValue ?? "none")")
                        .testColor(good: data.4 != nil)
                }
                ...
                Test(
                    ...
                    Text(verbatim: "Retrieved fuels: \(data.0?.displayValue ?? "none")")
                        .testColor(good: data.0?.displayValue == "plūka")
                ...
                Test(
                    ...
                    Text(verbatim: "Is unambiguous: \(data?.isUnambiguous ?? false)")
                        .testColor(good: data?.isUnambiguous == true)
                }
            }
            ...
            Test(
                "Constraints - keep the deepest constraint message - match at index",
                { () -> (WordModel?, LocalizedStringKey?) in
                    let w = WordTree(locale: .lv_LV)
            ...
            Test(
                "Constraints - don't show wrong message when word doesn't exist",
                { () -> (WordModel?, LocalizedStringKey?) in
                    let w = WordTree(locale: .lv_LV)
            ...
            Test(
                "Constraints - keep the deepest constraint message - contain",
                { () -> (WordModel?, LocalizedStringKey?) in
                    let w = WordTree(locale: .lv_LV)
