...
struct MultiCharacterModel: Hashable, Codable, Equatable, CustomDebugStringConvertible {
    ...
    init(values: [CharacterModel]) {
        guard !values.isEmpty else {
            fatalError("Character must have at least 1 value")
        }

        guard let firstLocale = values.first?.locale,
              values.allSatisfy({ $0.locale == firstLocale })
        else {
            fatalError(
                "All characters must have the same locale. Received: \(values.map { $0.locale.identifier }.joined(separator: ","))"
            )
        }
        self.values = values
    }
    ...
    var debugDescription: String {
        let inner = values.map { $0.debugDescription }.joined(separator: "|")
        return "[\(inner)]"
    }

    var displayValue: String {
        values.first?.displayValue ?? ""
    }
}
    ...
    struct Comparison: View {
        ...
        var body: some View {
            ...
            return VStack(alignment: .leading) {
                ...
                Text("\(leftM.debugDescription) \(match ? "==" : "!=") \(rightM.debugDescription)")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Group {
                ...
                Comparison(
                    desc: "Intersection match (case insensitive)",
                    left: ["c", "B"],
                    right: ["Z", "C"],
                    localeLeft: lvLV,
                    localeRight: lvLV,
                ...
                Comparison(
                    desc: "Intersection mismatch (diacritic sensitive)",
                    left: ["c", "Z"],
                    right: ["b", "Č"],
                    localeLeft: lvLV,
                    localeRight: lvLV,
