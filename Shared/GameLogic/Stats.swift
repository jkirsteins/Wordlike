...
struct Stats: RawRepresentable {
    ...
    init(played: Int, won: Int, maxStreak: Int, streak: Int, guessDistribution: [Int], lastWinAt: Date?) {
        self.played = played
        self.won = won
    ...
    func update(from game: GameState, with ps: TurnCounter) -> Stats {
        ...
        return Stats(
            ...
            won: won + (game.isWon ? 1 : 0),
            maxStreak: game.isWon ? max(newStreak, maxStreak) : maxStreak,
            streak: newStreak,
            guessDistribution: newDistribution,
            lastWinAt: game.isWon ? Date() : lastWinAt
        )
    }
...
extension Stats: Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.won = try values.decode(Int.self, forKey: .won)
        self.maxStreak = try values.decode(Int.self, forKey: .maxStreak)
        self.streak = try values.decode(Int.self, forKey: .streak)
        self.guessDistribution = try values.decode([Int].self, forKey: .guessDistribution)
        self.lastWinAt = try values.decode(Date?.self, forKey: .lastWinAt)
    }
