...
struct WordModel: Codable, Equatable, CustomDebugStringConvertible {
    ...
    var locale: Locale {
        ...
        guard let firstLocale = word.first?.locale,
              word.allSatisfy({ $0.locale == firstLocale })
        else {
    ...
    var debugDescription: String {
        word.map { $0.debugDescription }.joined(separator: "")
    }

    var isUnambiguous: Bool {
        word.allSatisfy { $0.values.count == 1 }
    }
    ...
    struct Comparison: View {
        ...
        var body: some View {
            ...
            return VStack(alignment: .leading) {
                ...
                Text(
                    verbatim: "\(left?.debugDescription ?? "none") \(match ? "==" : "!=") \(right?.debugDescription ?? "none")"
                )
