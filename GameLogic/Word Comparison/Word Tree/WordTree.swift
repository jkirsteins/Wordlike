...
private struct ErrorReason {
    let reason: String
    let depth: Int
}

private class Node: BranchNode, Hashable {
    ...
    init(from model: [RowModel]) {
        for row in model.filter({ $0.isSubmitted }) {
            for rowIx in 0 ..< row.word.count {
                ...
                guard let multiChar = row.char(guessAt: rowIx),
...
class WordTree {
    ...
    init(words: [WordModel], locale: Locale) {
        ...
        for word in words {
            ...
            _ = add(characters: word.word.map { $0.values.first! })
        }
    }

    func add(word: String) -> Bool {
        return add(characters: word.map {
            CharacterModel(value: $0, locale: locale)
        })
    }

    private func add(characters: [CharacterModel]) -> Bool {
